...
var substanceConfig = mysqlTable("substance_config", {
  id: int("id").autoincrement().primaryKey(),
...
var workouts = mysqlTable("workouts", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
var checkins = mysqlTable("checkins", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
var relapseLogs = mysqlTable("relapse_logs", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  fecha: timestamp("fecha").notNull(),
...
// server/routers.ts
var substanceConfigSchema = z2.object({
  tipoSustancia: z2.enum(["nicotina", "metanfetamina"]),
  unidad: z2.string().min(1),
  precioUnitario: z2.string().min(1),
  moneda: z2.string().min(1),
  fechaInicioAbstinencia: z2.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  ajustesPersonalizados: z2.string().optional()
});
var workoutSchema = z2.object({
  fecha: z2.string().regex(/^\d{4}-\d{2}-\d{2}$/),
  ejercicios: z2.string(),
  caloriasEstimadas: z2.number().optional()
});
var checkinSchema = z2.object({
  estadoAnimo: z2.number().min(1).max(10),
  craving: z2.number().min(1).max(10),
  notas: z2.string().optional(),
  biometrics: z2.string().optional()
});
var relapseLogSchema = z2.object({
  triggerTags: z2.string().optional(),
  contexto: z2.string().optional(),
  impactoEconomico: z2.string().optional(),
  reinicioCleanStreak: z2.number().optional()
});
var appRouter = router({
  ...
  substanceConfig: router({
    create: protectedProcedure.input(substanceConfigSchema).mutation(({ ctx, input }) => {
      return createSubstanceConfig({
        userId: ctx.user.id,
        ...input
      });
    }),
    get: protectedProcedure.query(({ ctx }) => {
      return getSubstanceConfig(ctx.user.id);
    }),
    update: protectedProcedure.input(z2.object({ id: z2.number(), ...substanceConfigSchema.shape })).mutation(({ input }) => {
      const { id, ...data } = input;
      return updateSubstanceConfig(id, data);
    })
  }),
  // Workouts
  workouts: router({
    ...
    update: protectedProcedure.input(z2.object({ id: z2.number(), ...workoutSchema.shape })).mutation(({ input }) => {
      const { id, ...data } = input;
      return updateWorkout(id, data);
    }),
    delete: protectedProcedure.input(z2.object({ id: z2.number() })).mutation(({ input }) => {
      return deleteWorkout(input.id);
    })
  }),
  // Check-ins
  checkins: router({
    create: protectedProcedure.input(checkinSchema).mutation(({ ctx, input }) => {
    ...
    list: protectedProcedure.query(({ ctx }) => {
      return getUserCheckins(ctx.user.id);
    }),
    update: protectedProcedure.input(z2.object({ id: z2.number(), ...checkinSchema.shape })).mutation(({ input }) => {
      const { id, ...data } = input;
  ...
  // Relapse Logs
  relapseLogs: router({
    create: protectedProcedure.input(relapseLogSchema).mutation(({ ctx, input }) => {
      return createRelapseLog({
        userId: ctx.user.id,
        fecha: /* @__PURE__ */ new Date(),
        ...input
      });
    ...
    list: protectedProcedure.query(({ ctx }) => {
      return getUserRelapseLogs(ctx.user.id);
    }),
    update: protectedProcedure.input(z2.object({ id: z2.number(), ...relapseLogSchema.shape })).mutation(({ input }) => {
      const { id, ...data } = input;
      return updateRelapseLog(id, data);
    })
  })
});
